...
class ChangeArrivalCityCommandTest {
    @Mock
    ...
    @InjectMocks
    ChangeArrivalCityCommand changeArrivalCityCommand;
    ...
    void setUp() {
        ...
        mockMvc = MockMvcBuilders.standaloneSetup(changeArrivalCityCommand)
                .dispatchOptions(true).build();
    }

    @Test
    @DisplayName("Invaid parameters entered. Validation failed")
    void execute() throws Exception {
        when(validation.validateId(anyString())).thenReturn(false);
        mockMvc.perform(post("/change_arrival_city")
                .param(PARAM_NAME_NEW_ARRIVAL_CITY, "city")
    ...
    void executeSuccess() throws Exception {
        ...
        doNothing().when(updateTourLogic).updateArrivalCity(anyString(), anyInt(), any());
        mockMvc.perform(post("/change_arrival_city")
                .param(PARAM_NAME_NEW_ARRIVAL_CITY, "city")
                ...
                .andExpect(model().attribute(ATTR_NAME_ARRIVAL_CITY,"city"));
    }
}